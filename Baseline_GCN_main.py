...
class Net(nn.Module):
    def __init__(self, in_features, hidden_dim, out_dim):
        ...
        self.gcn_layer1 = GCN_layer(hidden_dim, hidden_dim)
        self.gcn_layer2 = GCN_layer(hidden_dim, out_dim)
        
    def forward(self, g, features):
        h = self.linear(features)
        h = self.gcn_layer1(g, h)
        self.h = self.gcn_layer2(g, h)
        return self.h
...
net = Net(nfeats, args.hidden_dim, args.nclass)
...
for epoch in range(100):
    ...
    if (epoch%2==0):
        ...
        if auc_gnn > best_auc:
            best_auc = auc_gnn
            result = [epoch, auc_gnn, precision_gnn, recall_gnn, f1_gnn]
